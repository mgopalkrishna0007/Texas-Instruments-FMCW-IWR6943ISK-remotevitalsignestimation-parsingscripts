# gives the 30 seccond overlapped vitals waveform plot 
...
def plot_waveforms(heart_waveform_readings, breath_waveform_readings):
    ...
    for i in range(heart_waveforms.shape[1]):
        plt.plot(heart_waveforms[:, i], alpha=0.7, label=f'Waveform {i+1}')
    plt.xlabel('Sample Index')
    plt.ylabel('Amplitude')
    plt.legend()  # Add a legend to differentiate the waveforms

    # Plot breath waveforms
    plt.subplot(2, 1, 2)
    plt.title('Breath Waveforms Over 30 Readings')
    for i in range(breath_waveforms.shape[1]):
        plt.plot(breath_waveforms[:, i], alpha=0.7, label=f'Waveform {i+1}')
    plt.xlabel('Sample Index')
    plt.ylabel('Amplitude')
    plt.legend()  # Add a legend to differentiate the waveforms
